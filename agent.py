...
The image shows dots in a grid with lines connecting some of them. For EACH dot, write 'X' if the line passes through it, '.' if not.

Write row by row, top to bottom. One row per line. Separate with spaces. Trace the line carefully."""
        else:
            grid_prompt = f"""Look at this image carefully. The question is: {question}
